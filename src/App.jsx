...
export default function App() {
  ...
  const publish = async () => {
    ...
    if (editingId) {
      await updateDoc(doc(db, "articles", editingId), { title, content, category });
    } else {
  ...
  return (
    <div style={{
      ...
      <h1 style={{ textAlign: "center", fontSize: "36px", fontWeight: "900", color: "#020617" }}>
        📜 Historia de España
      </h1>
      ...
      {user && role === "owner" && (
        <div style={{ background: "#fff", padding: 20, marginTop: 20, borderRadius: 10 }}>
          <h2 style={{ color: "#020617", background: "#e2e8f0", padding: "10px", borderRadius: "8px", display: "inline-block", fontWeight: "900" }}>
            👤 Usuarios del sistema
          </h2>
          {users.map(u => (
            <div key={u.uid} style={{ marginBottom: 10, padding: 10, background: "#f8fafc", borderRadius: 8 }}>
            ...
            <div key={cat}>
              ...
              {articles.filter(a => a.category === cat).map(a => (
                <div key={a.id} style={{ background: "#fff", padding: 15, marginBottom: 15, borderRadius: 10 }}>
                  <h3>{a.title}</h3>
                  <p>{a.content}</p>
                  {a.image && <img src={a.image} style={{ maxWidth: "100%", marginTop: 10 }} alt={a.title} />}
                  {user && (
                    <div style={{ marginTop: 10 }}>
      ...
      {view === "links" && (
        <>
          <div style={{ marginTop: 40 }}>
            <h2 style={{ fontWeight: "900", fontSize: "26px", color: "#020617", background: "#e2e8f0", padding: "10px", borderRadius: "8px", display: "inline-block" }}>
              🔗 Enlaces de interés
            </h2>
            <div style={{ marginTop: 15 }}>
              {[
                ...
                <p key={link.name}>
                  <a href={link.url} target="_blank" rel="noopener noreferrer" style={{ fontWeight: "900", color: "#0f172a", fontSize: "16px" }}>
                    {link.name}
                  </a>
                </p>
              ))}
